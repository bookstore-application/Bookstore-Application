...
class ClientThread(Thread):
        ...
        try:
            while True:
                ...
                if data[0] == "login":
                    ...
                    bookid = data[1].strip()
                    title = data[2].strip()
                    author = data[3].strip().replace('\n', ' ').replace('\r', ' ')
                    genre = data[4].strip().replace('\n', ' ').replace('\r', ' ')
                    price = data[5].strip()
                    quantity = data[6].strip()
                    ...
                    if not data[1] or not data[2]:
                        msg = f"updatequantityfailure;fill in all fields".encode()
                        self.clientSocket.send(msg)
                        continue
                    ...
                    try:
                            topselling, authorsales = self.get_topselling_author(inventory_lines)
                            msg = f"report1;{topselling};{authorsales[topselling]}".encode()
                            self.clientSocket.send(msg)
                    except Exception as e:
                    ...
                    ClientThread.transaction_lock.acquire()
                    try:
                        with open("transactions.txt", "r") as t:
                            transaction_lines = t.readlines()
                    finally:
                        ClientThread.transaction_lock.release()

                    try:
                        busiest_cashier, transactions = self.get_busiest_cashier(transaction_lines)
                        msg = f"report3;{busiest_cashier.title()};{transactions} sales".encode()
                        self.clientSocket.send(msg)
                    except Exception as e:
                        msg = f"report3;{e}".encode()
                        self.clientSocket.send(msg)
                elif data[0] == "closeconnection":
    ...
    def get_topselling_author(self,lines):
        author_sales = {}
